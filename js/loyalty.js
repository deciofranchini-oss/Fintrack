...
const LOYALTY_CATALOG = [
  ...
  { type:'avianca_lifemiles', name:'Avianca LifeMiles', short:'Avianca', icon:'✈️', color:'#BF0000', category:'airline',
    site_url:'https://www.lifemiles.com', has_api:false,
    note:'Avianca — sem API pública' },
  { type:'american_aadvantage', name:'American AAdvantage', short:'AA', icon:'✈️', color:'#00539B', category:'airline',
    site_url:'https://www.aa.com/aadvantage', has_api:false,
    note:'American Airlines — sem API pública' },
  { type:'united_mileageplus', name:'United MileagePlus', short:'United', icon:'✈️', color:'#002244', category:'airline',
    site_url:'https://www.united.com/ual/en/us/fly/mileageplus.html', has_api:false,
    note:'United Airlines — sem API pública' },
  { type:'delta_skymiles', name:'Delta SkyMiles', short:'Delta', icon:'✈️', color:'#E21836', category:'airline',
    site_url:'https://www.delta.com/skymiles', has_api:false,
    note:'Delta Air Lines — sem API pública' },
  { type:'emirates_skywards', name:'Emirates Skywards', short:'Emirates', icon:'✈️', color:'#C8102E', category:'airline',
    site_url:'https://www.emirates.com/br/portuguese/skywards', has_api:false,
    note:'Emirates — sem API pública' },
  { type:'airline_other', name:'Outra companhia aérea', short:'', icon:'✈️', color:'#475569', category:'airline',
    site_url:null, has_api:false,
    note:'Qualquer outra companhia aérea não listada' },
  // ── Bancos / Cartões ──────────────────────────────────────────────────────
  ...
  { type:'ihg',         name:'IHG One Rewards', short:'IHG', icon:'🏰', color:'#007CC3', category:'hotel',
    ...
    note:'Sem API pública' },
  { type:'wyndham',     name:'Wyndham Rewards', short:'Wyndh', icon:'🏨', color:'#0050A0', category:'hotel',
    site_url:'https://www.wyndhamhotels.com/wyndham-rewards', has_api:false,
    note:'Wyndham Hotels & Resorts — sem API pública' },
  { type:'hyatt',       name:'World of Hyatt',  short:'Hyatt', icon:'🏨', color:'#1D3461', category:'hotel',
    site_url:'https://world.hyatt.com', has_api:false,
    note:'Hyatt Hotels — sem API pública' },
  { type:'radisson',    name:'Radisson Rewards', short:'Radiss', icon:'🏨', color:'#BE0000', category:'hotel',
    site_url:'https://www.radissonhotels.com/rewards', has_api:false,
    note:'Radisson Hotel Group — sem API pública' },
  { type:'hotel_other', name:'Outro hotel / rede hoteleira', short:'', icon:'🏨', color:'#475569', category:'hotel',
    site_url:null, has_api:false,
    note:'Qualquer outra rede hoteleira não listada' },
  // ── Varejo / Supermercado ─────────────────────────────────────────────────
...
function _loyEsc(s) {
  // Escapes HTML entities AND single quotes so the string is safe in onclick='...' attributes
  return String(s||'')
    .replace(/&/g,'&amp;')
    .replace(/</g,'&lt;')
    .replace(/>/g,'&gt;')
...
function _loyPickCatalog(type, icon, color, name, siteUrl, shortName) {
  ...
  // For "Outra companhia aérea": clear the name so user types their own
  const isOtherAirline = type === 'airline_other';
  const isOtherHotel   = type === 'hotel_other';
  const isOther = isOtherAirline || isOtherHotel;
  if (nameEl) {
    if (isOther) {
      nameEl.value = '';
      nameEl.placeholder = isOtherAirline
        ? 'Ex: Iberia Plus, Miles & More, Flying Blue…'
        : 'Ex: Best Western, Choice Hotels, Meliá…';
      nameEl.focus();
  ...
  if (nameEl && nameEl.value) _loyCheckDuplicate(nameEl.value, document.getElementById('loyProgId')?.value || '');
}
