...
// ─────────────────────────────────────────────────────────────
// Money input mask (digits -> cents), mobile-friendly
// Usage: attachMoneyMask(document.getElementById('txAmount'))
// Keeps user from needing to type ','
// ─────────────────────────────────────────────────────────────
function attachMoneyMask(inputEl) {
  if (!inputEl) return;
  if (inputEl.dataset && inputEl.dataset.moneyMaskAttached === '1') return;
  if (inputEl.dataset) inputEl.dataset.moneyMaskAttached = '1';
  ...
  const formatFromDigits = (digits, negative) => {
    digits = (digits || '').replace(/^0+(?=\d)/, '');
    const cents = digits ? parseInt(digits, 10) : 0;
    const txt = fmtBR.format(cents / 100);
  ...
  const applyMask = () => {
    ...
    inputEl.value = formatFromDigits(digits, negative);
  ...
  inputEl.addEventListener('beforeinput', (e) => {
    ...
    if (e.data === '-') {
      e.preventDefault();
      const cur = (inputEl.value || '').toString().trim();
      inputEl.value = cur.startsWith('-') ? cur.slice(1) : '-' + cur;
      applyMask();
      return;
    }
...
// Parse masked BR money input to Number (e.g., '1.234,56' -> 1234.56)
function parseMoneyMaskBR(value) {
